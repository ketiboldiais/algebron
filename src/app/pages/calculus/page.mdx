...
- A convergent sequence is a sequence with the following properties: There is some number ${c}$ such that, given a positive number ${\varepsilon,}$ the sequence's members all tend towards ${c,}$ with the majority of sequence members gathering around ${c.}$ I.e., most members are in the interval ${(a-\varepsilon, a + \varepsilon).}$
