...
- [Algebron](#algebron)
  ...
  - [CAS Modules](#cas-modules)
    - [simplify(expression)](#simplifyexpression)
    - [freeof(expr, subex)](#freeofexpr-subex)
...
Welcome to Algebron. On this page's sections, click to the right of the dollar sign (`$:`) to begin typing expressions. Hit "Enter" to execute the expression. Some of the REPL windows have code preset and entered to serve as examples.

## Basic Number Types & Arithmetic

Algebron supports basic arithmetic operations. Algebron's core arithmetic operations are displayed in the REPL below.
...
Primitive types in Algebron are expressions that cannot be reduced any further.

### Integers

Integers are the most basic type in Algebron. The largest possible integer in Algebron is ${9,007,199,254,740,991}$ or ${2^{53} - 1.}$ Any integer greater than this will be cast to a [big integer](#big-integers).

### Floating Point Numbers

Floating point numbers in Algebron are represented in the 64-bit format.

### Fractions

Fractions in Algebron are implemented as JavaScript objects.

### Big Integers

Integers larger than ${9,007,199,254,740,991}$ are automatically cast to big integers.

### Strings

Strings in Algebron are _always_ double quoted.
...
Any expression surrounded by the apostrophe (`'`) in Algebron is interpreted as a symbolic string. Symbolic strings are Algebron's data type for _algebraic expressions_, handled by Algebron's [CAS modules](#cas-modules).
...
### simplify(expression)
Where `expression` is a string or MathObj, simplifies the expression. Below is an example of using simplify directly in TypeScript.
...
### freeof(expr, subex)
Where `expr` and `subex` are strings or MathObjects, returns true if `expr` does not contain the `subex` and false otherwise.

~~~ts
const x = freeof('2x + 1', 'x');
console.log(x); // false

const y = freeof('3a^2 + 9b + c', 'y');
console.log(y); // true
~~~
